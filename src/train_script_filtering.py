...
# Keep lowest 30% by dG
threshold = train_set_df["dG"].quantile(0.3)
train_set_df = train_set_df[train_set_df["dG"] <= threshold]
...
if wandb_log:
    ...
    wandb.init(
        ...
        group="small_batch",
        name=f"time_split_filtering_final_{current_time}",
    )
        ...
        for batch in loader_train:
            ...
            if args.mixed_precision:
                with torch.amp.autocast(device_type="cuda"):
                    ...
                    # Standard masked sequence loss (no RAML weighting)
                    seq_loss = torch.sum(loss_tokenwise * mask_for_loss, dim=-1) / (
                        mask_for_loss.sum(dim=-1) + 1e-8
                    )
...
for epoch in range(main_args.num_epochs):
    ...
    torch.save(val_model.state_dict(), f"./model_weights/filtering_final/model_weights_{epoch}.pth")
